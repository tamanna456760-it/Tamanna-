...
from bd_king_ai import BDKingAI, AdvancedPredictor
import json
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            if filename.endswith('.csv'):
                ...
                return jsonify({'error': 'Unsupported format', 'system': 'BD-KING-R7'}), 400
...
def predict():
    try:
        ...
        if prediction_type == 'regression':
            ...
            return jsonify({'error': 'Invalid prediction type', 'system': 'BD-KING-R7'}), 400
...
def download_sample_data():
    ...
    sample_data = pd.DataFrame({
        ...
        'system_status': np.random.choice(['Optimal', 'Good', 'Warning', 'Critical'], 500, p=[0.6, 0.3, 0.08, 0.02]),
