...
class BDKingAI:
    ...
    def _statistical_analysis(self):
        ...
        for col in numerical_cols:
            ...
            stats_summary[col] = {
                ...
                "confidence_interval": [
                    round(data.mean() - 1.96 * data.std() / np.sqrt(len(data)), 4),
                    round(data.mean() + 1.96 * data.std() / np.sqrt(len(data)), 4)
                ]
            }
    ...
    def _detect_seasonality(self):
        ...
        for col in numerical_cols[:3]:  # Analyze first 3 columns
            ...
            if len(data) > 10:
                ...
                autocorr = [data.autocorr(lag=i) for i in range(1, min(10, len(data)//4))]
    ...
    def generate_ai_insights(self):
        ...
        if completeness < 90:
            insights.append(f"⚠️ Data completeness is {completeness}%. Consider data cleaning.")
        ...
        if patterns.get('seasonality'):
            for col, seasonality_info in patterns['seasonality'].items():
                if seasonality_info.get('has_seasonality'):
                    insights.append(f"📈 Seasonal pattern detected in {col} (strength: {seasonality_info['strength']})")
