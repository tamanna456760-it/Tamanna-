...
class PowerHubController:
    ...
    def _save_to_disk(self):
        try:
            ...
            Path(STORE_FILE).write_text(json.dumps({"saved_at": _now_ts(), "modules": data}, indent=2))
...
def create_api_app(controller: PowerHubController):
    app = Flask(__name__, template_folder=str(Path(__file__).parent.parent / "web" / "templates"))
    CORS(app)
