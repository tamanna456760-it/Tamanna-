...
class RNNSequenceAnalyzer(BaseNeuralNetwork):
    ...
    def __init__(self, config: Dict = None):
        ...
        self.logger.info(f"🔄 RNN Sequence Analyzer initialized for {self.num_features} features")
    ...
    def build_model(self) -> tf.keras.Model:
        ...
        try:
            ...
            for i, rnn_config in enumerate(self.config['architecture']['rnn_layers']):
                ...
                if (i < len(self.config['architecture']['rnn_layers']) - 1 or 
                    rnn_config.get('return_sequences', False)):
    ...
    async def analyze_sequences(self, sequences: np.ndarray, window_size: int = None) -> Dict:
        """
        ...
        try:
            ...
            analysis_results = {
                ...
                'anomaly_scores': await self._calculate_anomaly_scores(sequences, predictions)
            }
    ...
    async def _analyze_trends(self, predictions: np.ndarray) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'trend_stability': float(1 - np.std(max_probs))  # Higher is more stable
            }
        except Exception as e:
            self.logger.error(f"❌ Trend analysis failed: {e}")
            return {}

    async def _calculate_anomaly_scores(self, sequences: np.ndarray, predictions: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            anomaly_scores = 1 - max_confidence  # Lower confidence = higher anomaly score
