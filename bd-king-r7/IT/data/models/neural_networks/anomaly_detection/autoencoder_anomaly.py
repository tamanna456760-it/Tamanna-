...
class AutoencoderAnomalyDetector(BaseNeuralNetwork):
    ...
    def __init__(self, config: Dict = None):
        ...
        self.logger.info(
            f"🔄 Autoencoder Anomaly Detector initialized for {self.input_dim} features"
        )
    ...
    async def _calculate_reconstruction_threshold(self, X_normal: np.ndarray):
        ...
        try:
            ...
            reconstruction_errors = np.mean(
                np.square(X_normal - reconstructions), axis=1
            )
            ...
            if threshold_method == "percentile":
                ...
                self.reconstruction_threshold = np.mean(
                    reconstruction_errors
                ) + multiplier * np.std(reconstruction_errors)

            else:  # dynamic - will be calculated per batch
                self.reconstruction_threshold = np.mean(
                    reconstruction_errors
                ) + 2 * np.std(reconstruction_errors)
            ...
            self.logger.info(
                f"📊 Reconstruction threshold set to: {self.reconstruction_threshold:.6f}"
            )
    ...
    async def detect_anomalies(
        self, X: np.ndarray, dynamic_threshold: bool = False
    ) -> Dict:
        """
        ...
        try:
            ...
            anomaly_scores = (
                reconstruction_errors - self.normal_data_mean
            ) / self.normal_data_std

            # Determine threshold
            if dynamic_threshold:
                current_threshold = np.mean(reconstruction_errors) + 2 * np.std(
                    reconstruction_errors
                )
            else:
            ...
            anomaly_analysis = {
                ...
                "severity_scores": (
                    (anomaly_scores / np.max(anomaly_scores)).tolist()
            ...
            self.logger.info(
                f"🚨 Anomalies detected: {len(anomaly_indices)}/{len(X)} "
    ...
    async def analyze_anomaly_clusters(
        self, X: np.ndarray, n_clusters: int = 5
    ) -> Dict:
        ...
        try:
            ...
            kmeans = KMeans(
                n_clusters=min(n_clusters, len(anomaly_indices)), random_state=42
            )
                ...
                cluster_analysis.append(
                    {
                        ...
                        "average_anomaly_score": np.mean(
                            [
                                anomaly_results["anomaly_scores"][i]
                                for i in cluster_anomaly_indices
                            ]
                        ),
                    }
                )
    ...
    async def _save_model_weights(self, save_path: Path):
        ...
        try:
            ...
            model_data = {
                ...
                "normal_data_mean": (
                    float(self.normal_data_mean) if self.normal_data_mean else None
                ),
                "normal_data_std": (
                    float(self.normal_data_std) if self.normal_data_std else None
                ),
                "config": self.config,
            }
        ...
        try:
            ...
            if data_path.exists():
                with open(data_path, "r") as f:
                    ...
                    self.reconstruction_threshold = model_data.get(
                        "reconstruction_threshold"
                    )
