...
class LSTMAnomalyDetector(BaseNeuralNetwork):
    ...
    def __init__(self, config: Dict = None):
        ...
        self.logger.info(f"🔄 LSTM Anomaly Detector initialized for {self.n_features} features")
    ...
    def build_model(self) -> tf.keras.Model:
        ...
        try:
            ...
            for i, units in enumerate(self.config['architecture']['lstm_layers']):
                return_sequences = (i < len(self.config['architecture']['lstm_layers']) - 1 or 
                                  self.config['architecture'].get('return_sequences', False))
            ...
            for i, units in enumerate(self.config['architecture']['dense_layers']):
                ...
                if self.config['architecture'].get('dropout_rate', 0) > 0:
                    model.add(layers.Dropout(self.config['architecture']['dropout_rate']))
            
            # Output layer
            model.add(layers.Dense(self.n_features * self.prediction_horizon, name='output'))
    ...
    async def _calculate_prediction_threshold(self, X_normal: np.ndarray):
        ...
        try:
            ...
            if self.prediction_horizon == 1:
                ...
                for i in range(len(X_normal)):
                    if i + self.prediction_horizon < len(X_normal):
                        actual = X_normal[i+1:i+self.prediction_horizon+1].flatten()
            ...
            if threshold_method == 'percentile':
                ...
                self.prediction_threshold = np.mean(prediction_errors) + multiplier * np.std(prediction_errors)
            
            # Store normal prediction errors
            self.normal_prediction_errors = prediction_errors
            
            self.logger.info(f"📊 Prediction threshold set to: {self.prediction_threshold:.6f}")
    ...
    async def _execute_evaluation(self, X_test, y_test=None) -> Dict:
        ...
        try:
            ...
            if self.prediction_horizon == 1:
                ...
                for i in range(len(X_test)):
                    if i + self.prediction_horizon < len(X_test):
                        actual = X_test[i+1:i+self.prediction_horizon+1].flatten()
                        error = np.mean(np.square(actual - predictions[i]))
                        prediction_errors.append(error)
                prediction_errors = np.array(prediction_errors)
            ...
            evaluation_results = {
                ...
                'prediction_threshold': float(self.prediction_threshold) if self.prediction_threshold else None
            }
    ...
    async def detect_anomalies(self, sequences: np.ndarray, use_rolling_window: bool = None) -> Dict:
        """
        ...
        try:
            ...
            if use_rolling_window is None:
                use_rolling_window = self.config['anomaly_detection'].get('use_rolling_window', True)
            ...
            if self.prediction_horizon == 1:
                ...
                for i in range(len(sequences)):
                    if i + self.prediction_horizon < len(sequences):
                        actual = sequences[i+1:i+self.prediction_horizon+1].flatten()
                        error = np.mean(np.square(actual - predictions[i]))
                        prediction_errors.append(error)
                prediction_errors = np.array(prediction_errors)
            ...
            if use_rolling_window and len(prediction_errors) > 1:
                ...
                smoothed_errors = self._apply_rolling_window(prediction_errors, window_size)
            ...
            if self.normal_prediction_errors is not None:
                anomaly_scores = (errors_to_use - np.mean(self.normal_prediction_errors)) / np.std(self.normal_prediction_errors)
            else:
                anomaly_scores = errors_to_use / np.max(errors_to_use) if np.max(errors_to_use) > 0 else errors_to_use
            ...
            self.logger.info(f"🚨 LSTM Anomalies detected: {len(anomaly_indices)}/{len(sequences)} "
    ...
    async def forecast_sequences(self, initial_sequence: np.ndarray, steps: int = 10) -> np.ndarray:
    ...
    async def _save_model_weights(self, save_path: Path):
        ...
        try:
            ...
            model_data = {
                'prediction_threshold': float(self.prediction_threshold) if self.prediction_threshold else None,
                'normal_prediction_errors': self.normal_prediction_errors.tolist() if self.normal_prediction_errors is not None else None,
                'config': self.config
            }
