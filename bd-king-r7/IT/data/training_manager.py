...
class TrainingDataManager:
            ...
            with open(registry_file, 'r') as f:
                ...
                for dataset_id, dataset_info in registry_data.items():
                    self.dataset_registry[dataset_id] = DatasetInfo(
                        ...
                        created_date=datetime.fromisoformat(dataset_info['created_date']),
    ...
    async def register_dataset(self, dataset_config: Dict) -> str:
        ...
        try:
            ...
            self.logger.info(f"📁 Dataset Registered: {dataset_info.name} (ID: {dataset_id})")
    ...
    async def load_dataset(self, dataset_id: str, split: str = "all") -> Tuple[pd.DataFrame, Optional[pd.Series]]:
        """
    ...
    async def preprocess_dataset(self, dataset_id: str, preprocessing_steps: List[Dict]) -> str:
        """
        ...
        try:
            ...
            # Update dataset info
            dataset_info = self.dataset_registry[dataset_id]
            dataset_info.preprocessing_steps.extend([step['name'] for step in preprocessing_steps])
            dataset_info.size = len(df)
            
            await self._save_dataset_registry()
            
            self.logger.info(f"🔧 Dataset Preprocessed: {dataset_id} | Steps: {len(preprocessing_steps)}")
    ...
    async def _apply_preprocessing_step(self, df: pd.DataFrame, step: Dict) -> pd.DataFrame:
