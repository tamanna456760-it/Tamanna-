...
class TamannaAIApps {
    ...
    async executeWorkflow(appId, workflowName, inputData) {
        ...
        const workflow = app.config.workflows.find(w => w.properties.name === workflowName);
        if (!workflow) {
    ...
    preprocessData(data) {
        return {
            ...
            clean_text: typeof data.text === 'string' ? data.text.trim().toLowerCase() : data.text
        };
    }
    ...
    generateResponse(data) {
        return {
            ...
            response: {
                ...
                timestamp: new Date().toISOString(),
                context: data.sentiment ? `Based on ${data.sentiment.label} sentiment` : 'General response'
            }
        };
    }
    ...
    formatResponse(data) {
        return {
            ...
            metadata: {
                processing_steps: Object.keys(data).filter(key => key !== 'final_output'),
                app_version: '1.0.0'
            }
        };
    }
