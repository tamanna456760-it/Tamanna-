...
class AutoSyncBuildSystem:
    ...
    def initialize(self):
        ...
        try:
            ...
            # Setup logging
            setup_logging(self.config.get(
                "system", {}).get("log_level", "INFO"))
    ...
    async def start(self):
        ...
        try:
            ...
            asyncio.create_task(
                self.components["health"].monitor_continuously())
...
def main():
    ...
    parser.add_argument(
        "--config", default="config/sync_config.yaml", help="Path to configuration file"
    )
    parser.add_argument(
        "--log-level",
