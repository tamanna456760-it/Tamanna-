...
class TamannaMultilingualLexer:
    ...
    def _handle_number(self):
        ...
        while (self.position < len(self.source_code) and 
               (self.source_code[self.position].isdigit() or 
    ...
    def _handle_identifier(self):
        ...
        while (self.position < len(self.source_code) and 
               (self.source_code[self.position].isalnum() or 
                self.source_code[self.position] == '_' or
    ...
    def _handle_string(self):
        ...
        start_pos = self.position
        while (self.position < len(self.source_code) and 
               self.source_code[self.position] != quote_char):
    ...
    def _handle_operator(self):
        ...
        if char == '=' and next_char == '=':
            ...
            self.tokens.append(Token(TokenType.BORO_SOMAN, '>=', self.line, self.column))
            self.position += 2
            self.column += 2
            return
        elif char == '<' and next_char == '=':
            self.tokens.append(Token(TokenType.CHOTO_SOMAN, '<=', self.line, self.column))
            self.position += 2
            self.column += 2
            return
        elif char == '>' and next_char != '=':
...
class TamannaMultilingualParser:
    ...
    def parse_statement(self) -> ASTNode:
        ...
        if token.type in [TokenType.LIKHO, TokenType.PRINT]:
            ...
            if self.position + 1 < len(self.tokens) and self.tokens[self.position + 1].type in [TokenType.SOMAN, TokenType.NIRNAY]:
    ...
    def parse_comparison(self) -> ASTNode:
        ...
        while self.current_token().type in [
            TokenType.SOMAN, TokenType.ASOMAN, TokenType.BORO, 
            TokenType.CHOTO, TokenType.BORO_SOMAN, TokenType.CHOTO_SOMAN
        ]:
            token = self.current_token()
            self.eat(token.type)
    ...
    def parse_addition(self) -> ASTNode:
        ...
        while self.current_token().type in [TokenType.PLUS, TokenType.MINUS, TokenType.JOG, TokenType.BIYOG]:
            token = self.current_token()
            self.eat(token.type)
    ...
    def parse_multiplication(self) -> ASTNode:
        ...
        while self.current_token().type in [
            ...
            TokenType.GUN, TokenType.BHAG, TokenType.SHESH
        ]:
            token = self.current_token()
            self.eat(token.type)
