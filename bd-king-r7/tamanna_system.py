...
class TamannaLexer:
    ...
    KEYWORDS = {
        ...
        "while": TK.TK_WHILE,
        "for": TK.TK_FOR,
        "function": TK.TK_FUNCTION,
        "return": TK.TK_RETURN,
        "break": TK.TK_BREAK,
        "continue": TK.TK_CONTINUE,
        # Math Operators
        "যোগ": TK.TK_JOG,
        "বিয়োগ": TK.TK_BIYOG,
        "গুণ": TK.TK_GUN,
        ...
        "বড়": TK.TK_BORO,
        "ছোট": TK.TK_CHOTO,
        "বড়সমান": TK.TK_BORO_SOMAN,
        "ছোটসমান": TK.TK_CHOTO_SOMAN,
        # Logical
        "বা": TK.TK_O,
        "এবং": TK.TK_EBONG,
        "না": TK.TK_NA,
        ...
        "খালি": TK.TK_KHALI,
        # Network Keywords
        "নেটওয়ার্ক": TK.TK_NETWORK,
        "সার্ভার": TK.TK_SERVER,
        "ক্লায়েন্ট": TK.TK_CLIENT,
        "কানেক্ট": TK.TK_CONNECT,
        "সেন্ড": TK.TK_SEND,
        "রিসিভ": TK.TK_RECEIVE,
        "network": TK.TK_NETWORK,
        "server": TK.TK_SERVER,
        "client": TK.TK_CLIENT,
        "connect": TK.TK_CONNECT,
        "send": TK.TK_SEND,
        "receive": TK.TK_RECEIVE,
        # System Keywords
        "সিস্টেম": TK.TK_SYSTEM,
        "রান": TK.TK_RUN,
        "এক্সিকিউট": TK.TK_EXECUTE,
        "ফাইল": TK.TK_FILE,
        "system": TK.TK_SYSTEM,
        "run": TK.TK_RUN,
        "execute": TK.TK_EXECUTE,
        "file": TK.TK_FILE,
    }
    ...
    def _handle_string(self):
        ...
        self.tokens.append(
            Token(TK.TK_STRING, string_value, self.line, start_col))
        self.position += 1
        self.column += 1
    ...
    def _handle_operator(self):
        ...
        if char == "=" and next_char == "=":
            self.tokens.append(
                Token(TK.TK_SOMAN, "==", self.line, self.column))
            self.position += 2
            self.column += 2
            return
        elif char == "!" and next_char == "=":
            self.tokens.append(
                Token(TK.TK_ASOMAN, "!=", self.line, self.column))
            self.position += 2
            self.column += 2
            return
        elif char == ">" and next_char == "=":
            self.tokens.append(
                Token(TK.TK_BORO_SOMAN, ">=", self.line, self.column))
            self.position += 2
            self.column += 2
            return
        elif char == "<" and next_char == "=":
            self.tokens.append(
                Token(TK.TK_CHOTO_SOMAN, "<=", self.line, self.column))
            self.position += 2
            self.column += 2
            return
        elif char == ">":
    ...
    def create_server(port=8080):
        ...
        try:
            ...
            print(Color.colorize(
                f"Server started on port {port}", Color.GREEN))
...
class AutoBuildSystem:
    ...
    def build_project(self):
        ...
        self.logger.info(
            Color.colorize("Starting Tamanna project build...", Color.CYAN)
        )
        ...
        try:
            ...
            self.logger.info(
                Color.colorize(f"Found {len(hm_files)} .hm files", Color.GREEN)
            )
            ...
            self.logger.info(
                Color.colorize("Build completed successfully!", Color.GREEN)
            )
    ...
    def build_file(self, file_path: Path):
        ...
        try:
            self.logger.info(Color.colorize(
                f"Building: {file_path}", Color.YELLOW))
            ...
            self.logger.info(
                Color.colorize(f"Successfully built: {file_path}", Color.GREEN)
            )

        except Exception as e:
            self.logger.error(
                Color.colorize(f"Error building {file_path}: {e}", Color.RED)
            )
    ...
    def test_file(self, file_path: Path):
        ...
        try:
            ...
            if result.returncode == 0:
                self.logger.info(
                    Color.colorize(f"Test passed: {file_path}", Color.GREEN)
                )
            else:
                self.logger.warning(
                    Color.colorize(f"Test warnings: {file_path}", Color.YELLOW)
                )
                if result.stderr:
                    self.logger.error(
                        Color.colorize(f"Errors: {result.stderr}", Color.RED)
                    )

        except subprocess.TimeoutExpired:
            self.logger.error(Color.colorize(
                f"Test timeout: {file_path}", Color.RED))
        except Exception as e:
    ...
    def create_package(self):
        ...
        self.logger.info(Color.colorize(
            "Creating distribution package...", Color.CYAN))
        ...
        self.logger.info(Color.colorize(
            "Package created successfully!", Color.GREEN))
...
class TamannaInterpreter:
    ...
    def _execute_code(self, code: str):
        ...
        try:
            ...
            self.output.append(Color.colorize(
                f"Execution error: {e}", Color.RED))
...
class TamannaSystem:
    ...
    def __init__(self):
        ...
        print(
            Color.colorize(
                f"Tamanna System Started on {self.platform.upper()}", Color.MAGENTA
            )
        )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(
            Color.colorize(
                "Multi-Platform System (Windows + Linux + Android)", Color.CYAN
            )
        )
