...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // ---------- Local fallback bot ----------
    const reply = localBotReply(conversation, messages[messages.length - 1] ? messages[messages.length - 1].content : '');
...
function localBotReply(conversationText, lastUserContent) {
  ...
  if (lc.includes("hello") || lc.includes("hi")) return "Hi! Tamanna AI here. How can I assist your BD-KING-R7 today?";
  if (lc.includes("power") && lc.includes("normal")) return "Normal Power selected — lightweight mode active.";
  if (lc.includes("power") && lc.includes("deep")) return "Deep Power engaged — increasing compute and persistence.";
  if (lc.includes("deploy") || lc.includes("deploying")) return "To deploy, push to main and pipelines will trigger. Need a deployment script?";
  if (lc.includes("help")) return "I can: show power modes, run builds, or explain system files. Try: 'Show power modes' or 'Run auto-sync'.";
  if (lc.includes("tamanna")) return "Tamanna AI — your BD-KING-R7 assistant. Ask me to run scripts, explain code, or manage power levels.";
  if (lc.includes("who are you")) return "I am Tamanna AI, an assistant for the BD-KING-R7 system.";
