...
class GANAnomalyDetector(BaseNeuralNetwork):
    ...
    def __init__(self, config: Dict = None):
        ...
        self.logger.info("🔄 GAN Anomaly Detector initialized")
    ...
    def compile_model(
        ...
        try:
            ...
            self.gan.compile(
                optimizer=tf.keras.optimizers.Adam(
                    learning_rate=self.config["training"]["generator_lr"], beta_1=0.5
                ),
                loss="binary_crossentropy",
            )
    ...
    async def _execute_evaluation(self, X_test, y_test=None) -> Dict:
        ...
        try:
            ...
            evaluation_results = {
                ...
                ),  # Higher is better generator
                "discriminator_threshold": (
                    float(self.discriminator_threshold)
                    if self.discriminator_threshold
                    else None
                ),
            }
    ...
    async def detect_anomalies(self, X: np.ndarray) -> Dict:
        ...
        try:
            ...
            self.logger.info(
                f"🚨 GAN Anomalies detected: {len(anomaly_indices)}/{len(X)} "
        ...
        try:
            ...
            if data_path.exists():
                with open(data_path, "r") as f:
                    ...
                    self.discriminator_threshold = model_data.get(
                        "discriminator_threshold"
                    )
