...
class KnowledgeBaseManager:
    ...
    async def store_knowledge(
        ...
        try:
            ...
            self.logger.info(
                f"💡 Knowledge Stored: {knowledge_type.value} | ID: {knowledge_id}"
            )
    ...
    async def retrieve_knowledge(
        ...
        try:
            ...
            self.logger.debug(
                f"🔍 Knowledge Retrieved: {len(relevant_knowledge)} units"
            )
    ...
    async def update_knowledge_confidence(
        ...
        try:
            if knowledge_id in self.knowledge_store:
                ...
                self.logger.info(
                    f"📊 Knowledge Confidence Updated: {knowledge_id} -> {new_confidence}"
                )
    ...
    async def export_knowledge(self, export_path: str, format: str = "json"):
        ...
        try:
            ...
            export_file = (
                Path(export_path)
                / f"knowledge_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{format}"
            )
