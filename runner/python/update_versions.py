...
REPLACEMENTS = [
    ...
    (
        REPO_ROOT / "boot/tamanna.inf",
        r"VERSION_STRING\s*=\s*[\d\.]+",
        f"VERSION_STRING                 = {VERSION}",
    ),
    (
        REPO_ROOT / "boot/x200_100-efi/x2000_1000.inf",
        r"VERSION_STRING\s*=\s*[\d\.]+",
        f"VERSION_STRING                 = {VERSION}",
    ),
    (
        REPO_ROOT / "boot/EFI/tamanna/tamanna.inf",
        r"VERSION_STRING\s*=\s*[\d\.]+",
        f"VERSION_STRING                 = {VERSION}",
    ),
    (
        REPO_ROOT / "boot/x200_100-efi/tamanna.cfg",
        r"Version\s*=\s*[\d\.]+",
        f"Version    = {VERSION}",
    ),
    (REPO_ROOT / "boot/distas/stable", r"Version:\s*[\d\.]+", f"Version: {VERSION}"),
    (
        ROOT / "tamanna_config.hm",
        r"version\s*=\s*\"[\d\.]+\"",
        f'version = "{VERSION}"',
    ),
    (
        REPO_ROOT / "bd-king-r7_auto-fix_code.json",
        r"\"version\"\s*:\s*\"[\d\.]+\"",
        f'"version": "{VERSION}"',
    ),
    (
        REPO_ROOT / "tamanna/bd-king-r7_auto_code_fix.json",
        r"\"version\"\s*:\s*\"[\d\.]+\"",
        f'"version": "{VERSION}"',
    ),
]
