            ...
            if file["type"] == "file":
                ...
                if file_response.status_code == 200:
                    ...
                    if update_response.status_code == 200:
                        ...
                        print(
                            f'Failed to update file: {file["path"]}, status: {update_response.status_code}'
                        )
                    ai_report.append({"file": file["path"], "status": "updated"})
                else:
                    print(
                        f'Failed to fetch file: {file["path"]}, status: {file_response.status_code}'
                    )
                    ai_report.append({"file": file["path"], "status": "fetch_failed"})
