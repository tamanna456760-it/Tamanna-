...
class EnhancedNetworkSecurityMonitor:
    ...
    def __init__(self, config_file="security_config.json"):
        ...
        self.allowed_ports = set(
            self.config.get("allowed_ports", [80, 443, 22, 21, 53])
        )
                ...
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                    ...
                    if result == 0:
                        ...
                        if risk_level == "HIGH":
                            ...
                            self.logger.warning(
                                f"HIGH RISK: Port {port} ({service_info}) is open"
                            )
    ...
    def _assess_port_risk(self, port):
        ...
        medium_risk_ports = {
            ...
            5432,
            6379,
        }  # SSH, SMTP, POP3, PostgreSQL, Redis
    ...
    def continuous_monitoring(self, scan_interval=300):
        ...
        try:
            while True:
                ...
                if suspicious_conns:
                    self.logger.warning(
                        f"Found {len(suspicious_conns)} suspicious connections"
                    )
                ...
                if int(time.time()) % scan_interval == 0:
                    ...
                    if scan_results["security_risks"]:
                        self.logger.critical(
                            f"Security risks detected: {scan_results['security_risks']}"
                        )
    ...
    while True:
        ...
        if choice == "1":
            ...
            for port_info in results["open_ports"]:
                print(
                    f"Port {port_info['port']}: {port_info['service']} - Risk: {port_info['risk_level']}"
                )
