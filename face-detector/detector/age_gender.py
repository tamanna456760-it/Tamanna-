...
class AgeGenderDetector:
    ...
    def detect(self, image_path, output_path="static/uploads/age_gender.jpg"):
        ...
        for x, y, w, h in faces:
            ...
            cv2.putText(
                image, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2
            )

            results.append({"gender": gender, "age": age, "box": [x, y, w, h]})

        os.makedirs(os.path.dirname(output_path), exist_ok=True)

        cv2.imwrite(output_path, image)

        return {"faces": len(results), "results": results, "output": output_path}
